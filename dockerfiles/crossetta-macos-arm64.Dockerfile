FROM crazymax/osxcross:26.1-r0-debian AS osxcross
...
RUN /usr/bin/env UNATTENDED=1 osxcross-macports update-cache && UNATTENDED=1 osxcross-macports --$TARGET_ARCH \
    fake-install py313 py313-packaging xorg xrender curl-ca-bundle graphviz librsvg

# Install Qt 6.10 and dependencies
RUN /usr/bin/env UNATTENDED=1 osxcross-macports --$TARGET_ARCH install libGLU vulkan-headers vulkan-loader vulkan-utility-libraries qt610-qtbase qt610-qtsvg qt610-qtdeclarative qt610-qtshadertools
